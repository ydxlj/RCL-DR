...
class SparseMatrixDataset(Dataset):
    def __init__(self, drug_disease_matrix, apply_smote=None):
        ...
        # 打印训练集的正负样本数量
        print(f"Training set positive samples: {len(self.positive_pairs)}")
        print(f"Training set negative samples: {len(self.negative_pairs)}")
    ...
    def apply_smote(self):
        ...
        smote = SMOTE(sampling_strategy=1.0, random_state=56)  # 生成正样本的数量为负样本数量的30%
        ...
        # 打印应用 SMOTE 后的正负样本数量
        print(f"After SMOTE, training set positive samples: {len(self.positive_pairs)}")
        print(f"After SMOTE, training set negative samples: {len(self.negative_pairs)}")
...
class Trainer:
    ...
    def fit(self, train_data, val_data, epochs=1, batch_size=256, eval_step=1, save_model=True,
    ...
    def _train_epoch(self, train_loader, epoch_idx, loss_func=None, show_progress=False):
        ...
        total_loss = 0
        if show_progress:
            train_loader = tqdm(train_loader, desc=f"Epoch {epoch_idx + 1} Training", leave=True)  # 修改 leave=True
...
def calculate_auroc_aupr(model, val_data):
    model.eval()
    with torch.no_grad():
        # 确保 val_data 是一个稀疏矩阵
...
def train_multiple_models(train_data, val_data, model_params, optimizer_params, num_models=10):
    models = []
    for i in range(num_models):
        ...
        trainer.fit(train_data, val_data, epochs=100, batch_size=256, eval_step=1, save_model=False, show_progress=True)
        models.append(model)
    return models


def majority_voting_predict(models, val_data):
    all_predictions = []
    for model in models:
        model.eval()
        with torch.no_grad():
            val_drug_embeddings, val_disease_embeddings, _ = model.forward(model.norm_adj_mat)
            scores = torch.matmul(val_drug_embeddings, val_disease_embeddings.t())
            scores = torch.sigmoid(scores)
            all_predictions.append(scores.cpu().numpy())
...
def save_results(val_data, final_predictions, output_dir, repeat, fold):
    true_labels = val_data.toarray().flatten()
    true_labels_binary = (true_labels > 0).astype(int)
    ...
    with open(result_file, 'w') as f:
        ...
        for true_label, pred_score in zip(true_labels_binary, final_predictions):
            f.write(f"{true_label}\t{pred_score}\n")

    return true_labels_binary, final_predictions
...
def run_kfold_experiment(data, model_params, optimizer_params, kfold_params, num_repeats=2, apply_smote=True):
    ...
    for repeat in range(num_repeats):
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(data)):
            ...
            # 在训练集上应用 SMOTE 过采样
            if apply_smote:
                train_dataset = SparseMatrixDataset(train_data, apply_smote=True)
            ...
            # 训练多个模型
            models = train_multiple_models(train_data, val_data, model_params, optimizer_params, num_models=10)

            # 投票预测
            final_predictions = majority_voting_predict(models, val_data)

            # 保存结果
            y_true, y_pred = save_results(val_data, final_predictions, output_dir, repeat, fold)
            ...
            results.append({
                'repeat': repeat,
                'fold': fold,
                'best_valid_score': val_auroc,
                'best_valid_result': val_aupr
            })

    # 计算AUROC和AUPR的平均值和标准差
...
def main():
    # 读取药物疾病关联矩阵
    data_path = "D:\\13645583245\\文献代码\\代码\\dataset\\Fdataset\\Fdataset.mat"
