...
class Trainer:
    ...
    def fit(self, train_data, val_data=None, epochs=200, batch_size=256, eval_step=1, save_model=True,
        ...
        for epoch_idx in range(epochs):
            ...
            if epoch_idx % eval_step == 0:
                ...
                # 在每个 epoch 结束后评估模型并打印 AUROC 和 AUPR 等信息
...
def calculate_auroc_aupr(model, val_data):
    ...
    with torch.no_grad():
        ...
        # 获取验证数据集中的药物和疾病索引
        val_diseases = np.arange(val_data.shape[0])
        val_drugs = np.arange(val_data.shape[1])

        # 创建 interaction 字典
        interaction = {
            'drug': torch.tensor(val_diseases, dtype=torch.long).to(model.device),
            'disease': torch.tensor(val_drugs, dtype=torch.long).to(model.device)
        }

        # 使用 full_sort_predict 方法计算预测得分
...
def run_kfold_experiment(data, model_params, optimizer_params, kfold_params, num_repeats=2, apply_smote=True):
    ...
    for repeat in range(num_repeats):
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(data)):
            ...
            trainer = Trainer(model, optimizer, num_m_step=10,
                              device=torch.device("cuda" if torch.cuda.is_available() else "cpu"), patience=10)

            trainer.fit(train_data, val_data, epochs=200, batch_size=256, eval_step=1, save_model=True,
                        show_progress=True)
...
def main():
    # 读取药物疾病关联矩阵
    drug_dis_path = "/home/ubuntu/代码/dataset/Ldataset/lagcn/drug_dis.csv"
    drug_dis_df = pd.read_csv(drug_dis_path, header=None)
    drug_disease_matrix = drug_dis_df.values
